function [result] = solve_vector_lifting_shrinking(data, card, init_ML, init_CL, init_B_cell, global_ub, global_X, params)

    % SOLVE VECTOR-LIFTING SDP FOR ccMSSC

    % data: data matrix (n x d)
    % card: cardinality matrix (k x k)
    % init_ML: matrix of global must-link constraints (n_ml x 2)
    % init_CL: matrix of global cannot-link constraints (n_cl x 2)
    ...
    [blk, At, C, b, L, B_cell_cl, ~, n_cl_block, shr_B_cell, T] = build_vector_lifting_shrinking(W, card, init_ML, init_CL, init_B_cell);
    n = size(T, 1); % new problem size
    result.n = n;
    %result.T = T;
    
    init_n_ineq_block = zeros(k, 1);
    for j=1:k
        init_n_ineq_block(j) = size(shr_B_cell{j}, 2);
    end
    Bt = cell(1, k);
    l = [];
    for j=1:k
        B = [Y_zeros_cell(sum(init_n_ineq_block(1:(j-1)))+sum(n_cl_block(1:(j-1))), n+1), ... 
            shr_B_cell{j}, B_cell_cl{j}, ... 
    ...
    % recover xj blocks
    X = zeros(original_n, k);
    PI_cell = cell(1, k);
    for j=1:k
        Yj = Yopt{j}; % get j-th block
        X(:, j) = T' * Yj(2:n+1, 1);
        PI_cell{j} = T' * Yj(2:n+1, 2:n+1) * T;
    end
    ...
    % safe lower bound
    lb = max(lb1, lb2) + W_trace;
    % fprintf('\nLower bound = %10.9e \n', lb);
    
    % heuristic with the SDP solution
    [ub_X, Xass_X] = cardinality_heuristic_pairwise(X, [], data, card, init_ML, init_CL, params.gurobi_verbose);
    [ub_Z, Xass_Z] = cardinality_heuristic_pairwise([], PI_cell, data, card, init_ML, init_CL, params.gurobi_verbose);
