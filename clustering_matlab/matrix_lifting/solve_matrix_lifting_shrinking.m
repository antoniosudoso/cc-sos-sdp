function [result] = solve_matrix_lifting_shrinking(data, card, init_ML, init_CL, init_B_cell, global_ub, global_X, params)
    ...
    [blk, At, C, b, L, B_cell_cl, l_cl, shr_B_cell, T] = build_matrix_lifting_shrinking(W, card, init_ML, init_CL, init_B_cell);
    ...
    Bt = svec(blk, [shr_B_cell, B_cell_cl], 1);
    init_l = zeros(size(shr_B_cell, 2), 1);
    l = [init_l; l_cl];
    ...
    [ub_Z, Xass_Z] = cardinality_heuristic_pairwise([], Z, data, card, init_ML, init_CL, params.gurobi_verbose);
    ...
    result.ineq_list = [result.ineq_list; size(shr_B_cell, 2)];
